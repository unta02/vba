...
' Function to prompt user to load document content
Public Sub LoadAndAnalyzeDocument()
    ' Prompt user with options
    Dim response As VbMsgBoxResult
    response = MsgBox("Would you like to:" & vbCrLf & _
                     "YES - Load content from another Word document" & vbCrLf & _
                     "NO - Paste content from clipboard" & vbCrLf & _
                     "CANCEL - Cancel operation", _
                     vbYesNoCancel + vbQuestion, "Load Contract Content")
...
Private Sub LoadFromDocument()
    ' Create a FileDialog object
    Dim fd As FileDialog
    Set fd = Application.FileDialog(msoFileDialogFilePicker)
    
    With fd
        .Title = "Select Contract Document to Analyze"
        .Filters.Clear
        ...
        If .Show = -1 Then
            ...
            ' Store reference to current document (the .dotm)
            Dim currentDoc As Document
            Set currentDoc = ActiveDocument
            
            ' Open source document
            Set sourceDoc = Documents.Open(.SelectedItems(1))
            
            ' Clear current document content except for the last paragraph mark
            currentDoc.Content.Delete
            
            ' Copy content from source to current document
            sourceDoc.Content.Copy
            currentDoc.Content.Paste
            
            ' Close source document
            sourceDoc.Close False
            
            ' Make sure our .dotm is active
            currentDoc.Activate
...
Private Sub LoadFromClipboard()
    ...
    ' Paste clipboard content into current document
    currentDoc.Content.Paste
...
Public Sub AnalyzeWordDocument()
    ...
    ' Check if document is empty or just contains the template
    If Len(Trim(doc.Content.Text)) < 10 Then
        MsgBox "The document appears to be empty. Please load a contract first using the 'Load Contract' option.", vbExclamation
...
Public Function AnalyzeWithGemini(textToAnalyze As String) As String
    ...
    requestBody = "{""contents"": [{""parts"":[{""text"": """ & JsonEscape(textToAnalyze) & """}]},""safetySettings"": [{""category"": ""HARM_CATEGORY_DANGEROUS_CONTENT"", ""threshold"": ""BLOCK_ONLY_HIGH""}]}"
...
Public Function GetAnalysisPrompt(documentText As String) As String
    ...
    Dim part5 As String
    Dim part6 As String
    Dim part7a As String
    Dim part7b As String
    Dim part7c As String
    Dim part7d As String
    ...
    ' Split part7 into smaller chunks to avoid too many line continuations
    part7a = "FORMAT YOUR RESPONSE EXACTLY LIKE THIS:" & vbCrLf & _
             "PAYMENT TERMS:" & vbCrLf & _
             "[EXACT TEXT FROM DOCUMENT]" & vbCrLf & _
             "Analysis: [brief analysis]" & vbCrLf & vbCrLf & _
             "RATE CARDS:" & vbCrLf & _
             "[EXACT TEXT FROM DOCUMENT]" & vbCrLf & _
             "Analysis: [brief analysis]" & vbCrLf & vbCrLf
             
    part7b = "CLIENT TRAVEL AND EXPENSE POLICY:" & vbCrLf & _
             "[EXACT TEXT FROM DOCUMENT]" & vbCrLf & _
             "Analysis: [brief analysis]" & vbCrLf & vbCrLf & _
    ...
    part7c = "TERMINATION CLAUSES:" & vbCrLf & _
             "[EXACT TEXT FROM DOCUMENT]" & vbCrLf & _
             "Analysis: [brief analysis]" & vbCrLf & vbCrLf & _
    ...
    part7d = "INSURANCE PROVISIONS:" & vbCrLf & _
             "[EXACT TEXT FROM DOCUMENT]" & vbCrLf & _
             "Analysis: [brief analysis]" & vbCrLf & vbCrLf & _
    ...
    ' Combine all parts
    prompt = part1 & part2 & part3 & part4 & part5 & part6 & part7a & part7b & part7c & part7d
...
' Modify the AddMenuItemToWord sub to add both menu items
Public Sub AddMenuItemToWord()
    ...
    ' Try to find existing menu items first and remove them to avoid duplicates
    On Error Resume Next
    Word.Application.CommandBars("Menu Bar").Controls("Contract Analyzer").Delete
    Word.Application.CommandBars("Menu Bar").Controls("Load Contract").Delete
    On Error GoTo 0
    
    ' Add to the Menu Bar
    Set cb = Word.Application.CommandBars("Menu Bar")
    
    ' Add Load Contract button
    Set cbc = cb.Controls.Add(Type:=msoControlButton, Temporary:=True)
    With cbc
        .Caption = "Load Contract"
        .Style = msoButtonCaption
        .OnAction = "LoadAndAnalyzeDocument"
        .BeginGroup = True
    End With
    
    ' Add Analyze Contract button
    Set cbc2 = cb.Controls.Add(Type:=msoControlButton, Temporary:=True)
    With cbc2
        .Caption = "Analyze Contract"
        .Style = msoButtonCaption
        .OnAction = "AnalyzeWordDocument"
        .BeginGroup = False
    End With
    
    MsgBox "Contract Analyzer menu items have been added to the Menu Bar." & vbCrLf & _
           "1. Click 'Load Contract' to load a contract for analysis" & vbCrLf & _
           "2. Click 'Analyze Contract' to analyze the loaded contract", vbInformation
