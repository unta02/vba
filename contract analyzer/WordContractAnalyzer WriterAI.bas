...
' API key for Writer AI API
Private Const API_KEY As String = "fINtsD0Jk45gy2PJ4DPyN6xgPhn8oqqL"
Private Const API_URL As String = "https://api.writer.com/v1/chat"

' Add these new constants at the top with other constants
Private Const TOKEN_COUNT_URL As String = "https://api.writer.com/v1/tokenize"
Private Const MAX_TOKENS As Long = 128000  ' Maximum tokens for palmyra-x-004
...
' Add this new type for API response
Private Type APIResponse
    Content As String
    TokenCount As Long
...
Public Sub AnalyzeWordDocument()
    ...
    ' Call Writer AI API to analyze the text
    Word.Application.StatusBar = "Sending document to Writer AI for analysis..."
...
' Function to analyze text using Writer AI API
Public Function AnalyzeWithGemini(textToAnalyze As String) As String
    ...
    textToAnalyze = CleanTextForJson(textToAnalyze)
    
    ' Count tokens first
    apiResponse.TokenCount = CountTokens(textToAnalyze)
    If apiResponse.TokenCount > MAX_TOKENS Then
        AnalyzeWithGemini = "Error: Document exceeds maximum token limit (" & MAX_TOKENS & " tokens). Current document: " & apiResponse.TokenCount & " tokens."
        Exit Function
    End If
    
    ' Prepare the request body (JSON) for Writer AI
    requestBody = "{"
    requestBody = requestBody & """model"": ""palmyra-x-004"","
    requestBody = requestBody & """messages"": ["
    requestBody = requestBody & "{""role"": ""user"", ""content"": """ & textToAnalyze & """}"
    ...
    If Err.Number <> 0 Then
        apiResponse.ErrorMessage = "Error in HTTP request: " & Err.Description
        AnalyzeWithGemini = apiResponse.ErrorMessage
        Exit Function
    End If
    ...
    apiResponse.Content = ExtractTextFromWriterResponse(responseText)
    
    If apiResponse.Content <> "" Then
        ' Add token count to the response
        AnalyzeWithGemini = "Token Count: " & apiResponse.TokenCount & vbCrLf & vbCrLf & apiResponse.Content
    Else
        ' If extraction failed, return the error with raw response for debugging
        AnalyzeWithGemini = "Error parsing JSON response. Raw response: " & vbCrLf & responseText
    End If
    
    ' Log the response for debugging
    DebugLogResponse apiResponse
    
    On Error GoTo 0
End Function

' Function to count tokens
Private Function CountTokens(text As String) As Long
    Dim http As Object
    Dim requestBody As String
    Dim responseText As String
    ...
    ' Prepare the request body for token counting
    requestBody = "{""text"": """ & text & """}"
    
    ' Send the request
    On Error Resume Next
    http.Open "POST", TOKEN_COUNT_URL, False
    http.setRequestHeader "Content-Type", "application/json"
    http.setRequestHeader "Authorization", "Bearer " & API_KEY
    http.send requestBody
    ...
    ' Parse the response to get token count
    responseText = http.responseText
    CountTokens = ParseTokenCount(responseText)
    
    On Error GoTo 0
End Function

' Function to parse token count from response
Private Function ParseTokenCount(jsonString As String) As Long
    Dim count As Long
    Dim startPos As Long
    Dim endPos As Long
    
    ' Look for the token count in the response
    startPos = InStr(1, jsonString, """tokens"":")
    If startPos > 0 Then
        startPos = startPos + 9  ' Length of """tokens""":
        endPos = InStr(startPos, jsonString, "}")
        If endPos > startPos Then
            count = Val(Mid(jsonString, startPos, endPos - startPos))
        End If
    End If
    
    ParseTokenCount = count
End Function

' Function to log API response for debugging
Private Sub DebugLogResponse(response As APIResponse)
    Dim logFile As String
    Dim fileNum As Integer
    
    ' Create log file in temp directory
    logFile = Environ("TEMP") & "\WriterAI_Response_" & Format(Now, "yyyymmdd_hhnnss") & ".txt"
    
    fileNum = FreeFile
    Open logFile For Output As #fileNum
    
    Print #fileNum, "=== Writer AI API Response Log ==="
    Print #fileNum, "Timestamp: " & Now
    Print #fileNum, "Token Count: " & response.TokenCount
    Print #fileNum, "Error Message: " & response.ErrorMessage
    Print #fileNum, "=== Raw Response ==="
    Print #fileNum, response.RawResponse
    Print #fileNum, "=== Extracted Content ==="
    Print #fileNum, response.Content
...
Private Function CleanTextForJson(text As String) As String
    ...
    ' Replace problematic control characters but preserve line structure more carefully
    result = Replace(result, vbVerticalTab, " ")
    result = Replace(result, vbFormFeed, " ")
    result = Replace(result, vbBack, " ")
    
    ' Replace double quotes with single quotes
    result = Replace(result, """", "'")
    
    ' Replace line breaks with JSON line breaks correctly
    ...
    Dim i As Long
    For i = 1 To 31
        If i <> 10 And i <> 13 Then  ' Skip LF (10) and CR (13) as we already handled them
...
' Function to extract text from Writer AI response
Private Function ExtractTextFromWriterResponse(jsonString As String) As String
    On Error Resume Next
    
    Dim result As String
    result = ""
    
    ' Look for the content field in the choices array
    ...
    ' Return result
    ExtractTextFromWriterResponse = result
    
    On Error GoTo 0
End Function

' Create the detailed analysis prompt for Writer AI
...
' Extract the exact text from a section with improved clean-up
Private Function ExtractExactText(section As String) As String
    ...
    ' Remove the [EXACT TEXT FROM DOCUMENT] marker if present
    If InStr(text, "[EXACT TEXT FROM DOCUMENT]") > 0 Then
        text = Replace(text, "[EXACT TEXT FROM DOCUMENT]", "")
    End If
    
    ' Remove quotes if they're surrounding the text (often included in AI response)
    If Left(text, 1) = """" And Right(text, 1) = """" Then
        text = Mid(text, 2, Len(text) - 2)
    End If
    
    ' Remove asterisks that Writer AI sometimes adds for emphasis
    text = Replace(text, "**", "")
    text = Replace(text, "*", "")
    
    ' Handle literal newlines that Writer AI sometimes includes
    text = Replace(text, "\n", vbCrLf)
...
' Find text in a document with improved fuzzy matching
Private Function FindTextInDocument(doc As Word.Document, textToFind As String) As Word.Range
    Dim rng As Word.Range
    Dim found As Boolean
    
    ' Clean up search text
    textToFind = CleanSearchText(textToFind)
    
    ' Don't try to search for very short texts
    If Len(textToFind) < 5 Then
    ...
    ' First try: exact match (with length limit for Word's Find)
    Dim searchText As String
    If Len(textToFind) > 255 Then
        searchText = Left(textToFind, 255)
    Else
    ...
    ' Second try: Try shortened version (first 100 chars) for long text
    If Len(textToFind) > 100 Then
        searchText = Left(textToFind, 100)
    ...
    ' Third try: Split long text into chunks and try to find a significant portion
    If Len(textToFind) > 50 Then
        ' Try the first sentence or chunk
        Dim firstChunk As String
        Dim periodPos As Long
        
        periodPos = InStr(textToFind, ".")
        If periodPos > 10 Then
            firstChunk = Left(textToFind, periodPos)
        Else
            firstChunk = Left(textToFind, 50)
        End If
    ...
    ' Fourth try: Use an identifying keyword phrase from the text
    Dim keywordPhrase As String
    keywordPhrase = ExtractKeywordPhrase(textToFind)
    
    If Len(keywordPhrase) > 0 Then
        Set rng = doc.Content
        With rng.Find
            .text = keywordPhrase
            .Forward = True
...
Private Function CleanSearchText(text As String) As String
    Dim result As String
    
    ' Remove quotes and asterisks (Writer AI sometimes adds ** for emphasis)
    result = Replace(text, """", "")
    result = Replace(result, "'", "")
    result = Replace(result, "**", "")
    result = Replace(result, "*", "")
    
    ' Handle newlines properly (Writer AI sometimes includes "\n" literals)
    result = Replace(result, "\n", " ")
    ...
    CleanSearchText = result
End Function

' Extract a meaningful keyword phrase from text
Private Function ExtractKeywordPhrase(text As String) As String
    Dim result As String
    Dim i As Long
    Dim startPos As Long
    
    ' Find a good starting position - look for keywords
    startPos = 1
    
    ' Check for key contract-related terms
    ' Use static declaration to avoid "Constant expression required" error
    Dim keyTerms(16) As String
    keyTerms(0) = "reimburse"
    keyTerms(1) = "payment"
    keyTerms(2) = "invoic"
    keyTerms(3) = "expense"
    keyTerms(4) = "comply"
    keyTerms(5) = "service"
    keyTerms(6) = "travel"
    keyTerms(7) = "provision"
    keyTerms(8) = "personal"
    keyTerms(9) = "data"
    keyTerms(10) = "insurance"
    keyTerms(11) = "terminate"
    keyTerms(12) = "liability"
    keyTerms(13) = "require"
    keyTerms(14) = "MSA"
    keyTerms(15) = "agree"
    keyTerms(16) = "SOW"
    
    Dim bestTerm As String
    Dim bestPos As Long
    bestPos = Len(text) + 1
    
    ' Look for contract-specific keywords first
    For i = LBound(keyTerms) To UBound(keyTerms)
        Dim termPos As Long
        termPos = InStr(1, text, keyTerms(i), vbTextCompare)
        
        If termPos > 0 And termPos < bestPos Then
            bestPos = termPos
            bestTerm = keyTerms(i)
        End If
    Next i
    
    ' If we found a good keyword, start there
    If bestPos < Len(text) Then
        ' Find the beginning of the word
        While bestPos > 1 And Mid(text, bestPos - 1, 1) <> " "
            bestPos = bestPos - 1
        Wend
        startPos = bestPos
    Else
        ' Fallback to looking for capital letters as likely starts of important phrases
        For i = 1 To Len(text) - 20
            If Mid(text, i, 1) = " " And Mid(text, i + 1, 1) Like "[A-Z]" Then
                startPos = i + 1
                Exit For
            End If
        Next i
    ...
    ' Extract 20-30 character phrase starting from the identified position
    If startPos < Len(text) - 30 Then
        ' Find a good ending point (ideally at the end of a word)
        Dim endPos As Long
        endPos = InStr(startPos + 20, text, " ")
        
        If endPos > 0 And endPos < startPos + 30 Then
            result = Mid(text, startPos, endPos - startPos)
        Else
            result = Mid(text, startPos, 25)
        End If
    Else
        ' Just use the first 25 characters
        result = Left(text, 25)
    End If
